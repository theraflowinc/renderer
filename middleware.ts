...
const IMPORT_MAP = {
  ...
  "@tanstack/react-query": "https://esm.sh/@tanstack/react-query@5.62.0?external=react",
  ...
  "react-i18next": "https://esm.sh/react-i18next@15.1.0?external=react,i18next",
};
...
export async function middleware(request: NextRequest) {
  ...
  const isTheraflowHost = !!subdomainMatch;
  ...
  if (!isTheraflowHost) {
    ...
    return serveReactProject(sb, project, request);
  }
...
async function serveReactProject(
  sb: any,
  ...
  const { data: twRow } = await sb
    ...
    .select("data")
    .eq("project_id", project.id)
    .eq("collection", "tailwind_config")
    .maybeSingle();
  const tailwindConfigScript = (twRow?.data as any)?.script ?? "";
...
<head>
  ...
  ${tailwindConfigScript ? `<script>window.tailwind=window.tailwind||{};window.tailwind.config=${tailwindConfigScript};</script>` : ""}
